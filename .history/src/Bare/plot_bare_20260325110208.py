...
def discover_mus_from_outputs(output_prefix: str, beta: float):
    patterns = [f"{output_prefix}_beta={beta}_mu=*.npz"]

    if output_prefix.endswith(os.sep) or os.path.isdir(output_prefix):
        patterns.append(os.path.join(output_prefix, f"_beta={beta}_mu=*.npz"))

    files = []
    for pattern in patterns:
        files.extend(glob.glob(pattern))

    # Fallback: if exact beta string formatting differs, search all matching files in the output directory.
    if len(files) == 0:
        if os.path.isdir(output_prefix):
            files = glob.glob(os.path.join(output_prefix, "*_beta=*_mu=*.npz"))
        else:
            files = glob.glob(f"{output_prefix}*_beta=*_mu=*.npz")
    ...
    if len(mus) == 0:
        raise ValueError(
            ...
            f"Expected files matching one of: {patterns}"
        )
