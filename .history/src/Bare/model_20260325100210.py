...
def _lookup_high_symmetry_point(model, label: str) -> list:
    if label not in model.bz.HighSymPoints:
        available = ", ".join(sorted(model.bz.HighSymPoints.keys()))
        raise ValueError(f"Unknown HighSymPoint '{label}'. Available points: {available}")
    return _as_float_triplet(model.bz.HighSymPoints[label])
...
def normalize_k_points(model, k_points: list):
    ...
    for index, point in enumerate(k_points):
        if isinstance(point, str):
            resolved_points.append(_lookup_high_symmetry_point(model, point))
            inferred_labels.append(point)
            continue
