...
function get_mu_values(input)
    ...
    if haskey(input, "fillings") && haskey(input["fillings"], "values")
        error("Input defines fillings but has no resolved mus.values. Run src/Bare/run_bare.jl once to generate mus from fillings.")
    end

    error("Input must define mus.values, or fillings.values that have been resolved by the bare stage.")
end
...
function collect_interaction_cases(interaction_data)
    interaction_values = interaction_data["values"]

    if interaction_values isa AbstractDict
        return [(String(label), value) for (label, value) in interaction_values]
    end

    if interaction_values isa AbstractVector
        labels = haskey(interaction_data, "labels") ? interaction_data["labels"] : nothing
        cases = Tuple{String, Any}[]

        for (index, value) in enumerate(interaction_values)
            label = labels === nothing ? "interaction_$(index)" : String(labels[index])
            push!(cases, (label, value))
        end

        return cases
    end

    error("Unsupported interactions format: values must be a Dict or Vector.")
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    interaction_cases = collect_interaction_cases(interaction_data)
